...
## Loss for Data-free Knowledge Distillation
class dafl_loss(nn.Module):
    def __init__(self, old_class, new_class):
        super(dafl_loss, self).__init__()
        self.old_class = old_class
        self.new_class = new_class

    def forward(self, output, teacher_score):
        p = F.log_softmax(output, dim=1)[:, :self.old_class]
        q = F.softmax(teacher_score, dim=1)
        l_kl = F.kl_div(p, q, reduction='sum') / output.shape[0]
        return l_kl