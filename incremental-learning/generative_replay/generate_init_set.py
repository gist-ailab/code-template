...
def main(rank, option, task_id, save_folder, init_path):
    ...
    new_model_list = load_model(option, num_class, rank)
    ...
    criterion = load_loss(option, old_class, new_class, task_id=0)
    ...
    new_model_list[0].load_state_dict(save_module.save_dict['model'][0])
    ...
    if ddp:
        ...
        new_model_list[0].to(rank)
        new_model_list[0] = DDP(new_model_list[0], device_ids=[rank])
        new_model_list[0] = apply_gradient_allreduce(new_model_list[0])

    else:
        if multi_gpu:
            new_model_list[0] = nn.DataParallel(new_model_list[0]).to(rank)
        else:
            new_model_list[0] = new_model_list[0].to(rank)
